...
--
-- Three trigger functions depend on vault secrets and, until now, all
-- three did the same thing when a secret was missing: RAISE NOTICE and
-- return. A NOTICE goes to the Postgres log where nobody looks, so the
-- practical effect was that notifications silently didn't happen -- which
-- is exactly what happened on the first live sign-up test.
--
-- This replaces the silent skip with a durable row in system_alerts,
-- surfaced as a banner in the intranet. A background job that stops
-- working should be loud, not quiet.
...
-- One open alert per kind at a time, so a repeatedly-failing job doesn't
-- bury the dashboard in thousands of identical rows.
...
-- ---------------------------------------------------------------
-- Helper: record an alert (idempotent per kind)
-- ---------------------------------------------------------------
...
-- ---------------------------------------------------------------
-- Rewire the three vault-dependent functions to alert, not whisper
-- ---------------------------------------------------------------
...
BEGIN
  ...
  -- Clear any previous alert of this kind now that it's working again.
