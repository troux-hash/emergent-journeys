-- Notify on every new operator sign-up.
--
-- Fires from the database rather than the browser so a notification
-- cannot be missed because the visitor closed the tab, lost connection,
-- or posted straight to the API. The edge function handles all three
-- channels (team email, team WhatsApp, operator acknowledgement) and
-- never throws, so a notification failure can never roll back the
-- sign-up itself.
