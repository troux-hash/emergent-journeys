...
AS $$
DECLARE op RECORD; v_place TEXT;
BEGIN
  IF NOT public.has_role(auth.uid(), 'admin') THEN RAISE EXCEPTION 'admin role required'; END IF;
  SELECT o.name, o.city, o.country INTO op FROM public.operators o WHERE o.id = p_operator_id;
...
BEGIN
  ...
  FOR r IN SELECT * FROM public.generate_baseline_queries(p_operator_id) LOOP
    IF NOT EXISTS (
      ...
      WHERE d.operator_id = p_operator_id AND d.phase = 'baseline'
        AND d.engine = r.engine AND d.query_text = r.query_text
    ) THEN
      INSERT INTO public.discoverability_tests (operator_id, phase, engine, query_text, notes)
      VALUES (p_operator_id, 'baseline', r.engine, r.query_text,
...
RETURNS TABLE (
  all_checks_passed BOOLEAN, identity_verified BOOLEAN, photo_gps_verified BOOLEAN,
  whatsapp_verified BOOLEAN, payout_verified BOOLEAN, has_room_types BOOLEAN,
  has_gps BOOLEAN, baseline_count INTEGER, has_baselines BOOLEAN, blocking_reasons TEXT[]
) LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = public
AS $$
...
BEGIN
  ...
  IF NOT op.identity_verified THEN v_reasons := v_reasons || 'Identity and ownership not confirmed — do not publish a business that has not agreed to be listed.'; END IF;
  IF NOT op.photo_gps_verified THEN v_reasons := v_reasons || 'Photos not cross-checked against GPS location.'; END IF;
  IF NOT op.whatsapp_verified THEN v_reasons := v_reasons || 'WhatsApp number not confirmed to reach a real person.'; END IF;
  IF NOT op.payout_verified THEN v_reasons := v_reasons || 'Payout account not registered.'; END IF;
  IF v_rooms = 0 THEN v_reasons := v_reasons || 'No room types with prices — the page would publish with no bookable offer.'; END IF;
  IF op.lat IS NULL OR op.lng IS NULL THEN v_reasons := v_reasons || 'No GPS coordinates — map and location schema would be missing.'; END IF;
  IF v_baselines = 0 THEN v_reasons := v_reasons || 'No AI-discoverability baseline captured. Once this page is indexed the "before" measurement is gone permanently.'; END IF;
  RETURN QUERY SELECT
